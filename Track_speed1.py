...
lines  = [
    {'Title' : 'Line1', 'Cords' : [(400, 150), (800, 150)]}
...
def draw_boxes(img, bbox, object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if id not in set(data_deque):  
          data_deque[id] = deque(maxlen= 10000000)
...
if __name__=='__main__':
    ...
    #frame_count = 0
    #fps = 30.0
    filename = "./count_demo_out.mp4"
    #fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    #fourcc = cv2.VideoWriter_fourcc('M','J','P','G')
    #vid_writer = cv2.VideoWriter(filename, fourcc, fps, (int(width), int(height)))
    
    #vid_writer.open(filename, codec, fps, (int(width), int(height));
    vid_writer = cv2.VideoWriter(filename, cv2.VideoWriter_fourcc(*"mp4v"), fps, (int(width), int(height))
    ) # open one video
